...
//     setSubmitting(true)
//     try {
//       // Update task status to 'review' and completion to 100%
//       await api.put(`/intern/tasks/${task.id}`, {
//         status: 'review',
//         completion_percentage: 100,
//       })
//       onSubmit(`Task "${task.title}" submitted for review!`)
//       onClose()
//     } catch (err) {
...
//             <div>
//               <div style={{ fontSize: 11, color: '#6b7280' }}>DUE DATE</div>
//               <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>{task.due_date}</div>
//             </div>
//             <div>
//               <div style={{ fontSize: 11, color: '#6b7280' }}>ASSIGNED BY</div>
//               <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>{task.assigner?.name || '—'}</div>
//             </div>
//           </div>
...
//               placeholder={`Describe what you completed, any blockers you faced, links to your work (GitHub, Figma, docs), or anything else your manager should know...`}
//               style={{
...
//             <label style={{ display: 'block', fontWeight: 600, fontSize: 14,
//               color: '#374151', marginBottom: 8 }}>
//               📎 Attachments <span style={{ fontWeight: 400, color: '#9ca3af' }}>(optional)</span>
//             </label>
...
//                 PDF, images, zip, docs — any format
//               </div>
//             </div>
//             <input
//               ref={fileRef}
...
//             borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#92400e',
//           }}>
//             ⚠️ Submitting will mark this task as <strong>Under Review</strong> and notify your manager.
//           </div>
//         </div>
...
//   const [updateForm,  setUpdateForm]  = useState({})
//   const [successMsg,  setSuccessMsg]  = useState('')
//   const [submitTask,  setSubmitTask]  = useState(null) // task being submitted
...
//     try {
//       await api.put(`/intern/tasks/${taskId}`, updateForm)
//       setSuccessMsg('Task progress updated!')
//       setUpdating(null)
//       fetchTasks()
//     } catch (err) {
...
//         <div>
//           <h2>My Tasks</h2>
//           <p>Tasks assigned to you by your manager. Update your progress here.</p>
//         </div>
//       </div>
...
//       {/* Stats */}
//       <div className="stats-grid" style={{ marginBottom: 24 }}>
//         <div className="stat-card"><div className="stat-label">Total Tasks</div><div className="stat-value">{total}</div></div>
//         <div className="stat-card"><div className="stat-label">In Progress</div><div className="stat-value">{inProgress}</div></div>
//         <div className="stat-card"><div className="stat-label">Completed</div><div className="stat-value">{completed}</div></div>
//         <div className="stat-card">
//           <div className="stat-label" style={{ color: overdue > 0 ? '#dc2626' : undefined }}>Overdue</div>
...
//             return (
//               <section key={task.id} className="card" style={{
//                 borderLeft: `4px solid ${isOverdue ? '#dc2626' : task.status === 'completed' ? '#16a34a' : task.status === 'review' ? '#7c3aed' : '#00b1b4'}`,
//               }}>
//                 <div style={{ display: 'flex', justifyContent: 'space-between',
//                   alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
...
//                     <div style={{ display: 'flex', gap: 20, fontSize: 13,
//                       color: '#6b7280', flexWrap: 'wrap' }}>
//                       <span>📅 Due: <strong style={{ color: isOverdue ? '#dc2626' : '#374151' }}>{task.due_date}</strong></span>
//                       {task.tech_stack && <span>🛠 {task.tech_stack}</span>}
//                       {task.assigner  && <span>👤 Assigned by: <strong>{task.assigner.name}</strong></span>}
//                     </div>
...
//                     borderRadius: 10, border: '1px solid #e2e8f0',
//                   }}>
//                     <h4 style={{ margin: '0 0 14px', fontSize: 14 }}>Update Your Progress</h4>
//                     <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-end' }}>
//                       <div className="form-group" style={{ margin: 0 }}>
//                         <label>Status</label>
//                         <select name="status" value={updateForm.status} onChange={handleUpdateChange}>
...
//                       <div style={{ display: 'flex', gap: 8 }}>
//                         <button className="btn-primary btn-small"
//                           onClick={() => submitUpdate(task.id)}>Save</button>
//                         <button className="btn-secondary btn-small"
//                           onClick={() => setUpdating(null)}>Cancel</button>
//                       </div>
//                     </div>
...
//         <div className="stat-card"><div className="stat-label">In Progress</div><div className="stat-value">{inProgress}</div></div>
//         <div className="stat-card"><div className="stat-label">Completed</div><div className="stat-value">{completed}</div></div>
//         <div className="stat-card"><div className="stat-label" style={{ color: overdue > 0 ? '#dc2626' : undefined }}>Overdue</div>
//           <div className="stat-value" style={{ color: overdue > 0 ? '#dc2626' : undefined }}>{overdue}</div>
//         </div>
//       </div>
...
//             return (
//               <section key={task.id} className="card" style={{
//                 borderLeft: `4px solid ${isOverdue ? '#dc2626' : task.status === 'completed' ? '#16a34a' : '#00b1b4'}`,
//               }}>
//                 <div style={{ display: 'flex', justifyContent: 'space-between',
//                   alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
...
//                           <option value="todo">Todo</option>
//                           <option value="in_progress">In Progress</option>
//                           <option value="review">Review / Done — pending check</option>
//                           <option value="completed">Completed</option>
//                           <option value="blocked">Blocked</option>
//                         </select>
//                       </div>
...
//                         <label>Completion — {updateForm.completion_percentage}%</label>
//                         <input
//                           type="range" name="completion_percentage"
//                           min={0} max={100} step={5}
//                           value={updateForm.completion_percentage}
//                           onChange={handleUpdateChange}
//                           style={{ width: '100%' }}
